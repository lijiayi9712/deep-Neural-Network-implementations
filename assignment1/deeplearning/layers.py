...
def affine_forward(x, w, b):
  ...
  for i in x[0].shape:
    shape_1d *= i
  x_reshaped = x.reshape(N, shape_1d)
...
def batchnorm_forward(x, gamma, beta, bn_param):
  ...
  if mode == 'train':
    ...
    #############################################################################
    #x is the sample, we want sample mean for each feature
    ...
    out = x*(gamma/std)+beta-running_mean*(gamma/std)
    #############################################################################
    #                             END OF YOUR CODE                              #
    #############################################################################
  else:
...
def batchnorm_backward(dout, cache):
  ...
  dx_hat = dout*gamma
  x_hat_div = np.sum(dx_hat*xmu, axis=0)
  inv_var = (1./np.sqrt(sample_var + eps))
  dx1 = dx_hat* inv_var
  dsqrtvar = -1./(sqrtvar**2)*x_hat_div
  dvar = np.sum(dx_hat*xmu, axis=0)*(-0.5) * 1. /np.sqrt(sample_var+eps)**3
  dx2 = 2 * xmu * (1./N * np.ones((N,D))*dvar)
  dmu = -1 * np.sum(dx1 + dx2, axis=0)
  dx3 = 1./N*np.ones((N,D))*dmu
  dx = dx1 + dx2 + dx3
...
def conv_forward_naive(x, w, b, conv_param):
  ...
  for i in range(H_prime):
      for j in range(W_prime):
            ...
            for m in range(F):
                conv_value = np.sum(w[m]*x_slice, axis = (1, 2, 3))+b[m]
                out[:, m, i, j] = conv_value
  ...
  for i in range(dH):
      for j in range(dW):
        ...
        for n in range(N):
            #print(w.shape)
            #print(dout.shape)
            #dx_pad[n, :, i*stride:(i*stride+HH), j*stride:(j*stride+WW)] += np.sum(w*dout[n, :, i, j], axis = 0)
...
def max_pool_forward_naive(x, pool_param):
  ...
  for i in range(H_prime):
      for j in range(W_prime):
          for k in range(N):                                                                
              x_pool = x[k, :, i*stride:(i*stride+HH), j*stride:(j*stride+WW)]    
              out[k, :, i, j] = np.max(x_pool, axis=(1,2))                                                                   
  #############################################################################
  #                             END OF YOUR CODE                              #
  #############################################################################
  cache = (x, pool_param)
...
def max_pool_backward_naive(dout, cache):
  ...
  for i in range(H_prime):
      for j in range(W_prime):
            for n in range(N):
                for c in range(C):
                    x_pool = x[n, c, i*stride:(i*stride+HH), j*stride:(j*stride+WW)]                                               
                    x_pool_reshaped = np.reshape(x_pool, HH*WW)
                    temp_output = np.zeros(x_pool_reshaped.shape) 
                    temp_output[np.argmax(x_pool_reshaped)] = 1
                    output = np.reshape(temp_output, (HH, WW))
                    dx[n,c, i*stride:i*stride+HH, j*stride:j*stride+WW] = output*dout[n, c, i, j]       
  #############################################################################
  #                             END OF YOUR CODE                              #
  #############################################################################
  return dx
...
def spatial_batchnorm_forward(x, gamma, beta, bn_param):
  ...
  out, cache = batchnorm_forward(x_new.reshape((N*H*W, C)), gamma, beta, bn_param)
...
def spatial_batchnorm_backward(dout, cache):
  ...
  #############################################################################
  N,C,H,W = dout.shape
  dout_new = dout.transpose(0,3,2,1)
  out, dgamma, dbeta = batchnorm_backward_alt(dout_new.reshape((N*H*W,C)),cache)
  dx = out.reshape(N,W,H,C).transpose(0,3,2,1)
